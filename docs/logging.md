...
| `hopset.sense` | TX (`DEVOURER_TX_SENSE`) | t, v, role, slot, round, gen, base_idx, ch, phase (pre\|post), probe, window_us (MEASURED, barrier-read-end→read-end), settle_us, read_us, valid_fa + cca_ofdm/fa_ofdm, valid_cck + cca_cck/fa_cck (gated SEPARATELY — a scout-sourced sample has a real OFDM delta with no CCK reset behind it, see `RxEnergy::valid_cck`), cca_rate/fa_rate (per ms, summing only the halves that are valid), valid_igi + igi, valid_nhm + nhm_busy/nhm_dur, flags "0x…" (`TxSenseFlag`), scored, occ — one quiet-window observation |
...
| `survey.dwell` | chanscout (`src/chanmig/SurveyJsonl.h` owns the schema) | v, seq, chan, round, plan "0x…", start_ms, end_ms, retune_us, settle_ms, observe_ms, cca_ofdm\|null, fa_ofdm\|null (gated on `valid_fa`), cca_cck\|null, fa_cck\|null (gated SEPARATELY on `valid_cck` — a scout-sourced dwell has a real OFDM delta with no CCK reset behind it, so its CCK half is null rather than a fake 0; see `RxEnergy::valid_cck`), igi\|null, nhm_busy\|null, nhm_peak, nhm_dur, nhm[12], frames, rssi_mean, rssi_max, snr_mean, snr_min, evm_mean\|null, dvr_frames, dvr_air_us, oth_air_us (canonical-SA vs foreign decoded-airtime split), flags (validity bitmask), scout_id "0x…", agen |
